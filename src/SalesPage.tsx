...
export default function SalesPage({ colors, darkMode, onSelectChat, selectedChat }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API_BASE}/api/sales-chats`)
      .then(res => {
  ...
  return (
    <div style={{ padding: "36px 22px", width: "100%" }}>
      <h2 style={{ color: colors.red, fontWeight: 700, marginBottom: 18 }}>Sales Chats</h2>
      {error ? (
